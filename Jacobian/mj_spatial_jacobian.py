...
import os
from spatial_jacobian import spatial_jacobian
from spatial_jacobian_torque import spatial_jacobian_torque
...
while not glfw.window_should_close(window):
    ...
    # mj end-effector torque
    jacp = np.zeros(shape=(3,model.nv))  
    jacr = np.zeros(shape=(3,model.nv))
    body_id = model.body("wrist_3_link").id
    point_pos = data.site("end_effector").xpos
    mj.mj_jac(model, data, jacp, jacr, point_pos, body_id)
    mj_torque = np.vstack([jacp, jacr])
    
    # py jacobian
    *_, J_g6 = spatial_jacobian_torque(q)
    je_t = J_g6
