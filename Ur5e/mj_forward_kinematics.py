...
while not glfw.window_should_close(window):
    ...
    while (data.time - time_prev < 1.0/60.0):
        ...
        sol, robot = forward_kinematics(q.copy())
        # pose calculation using local fk
        print(f"ee pose => {sol.end_eff_pos}")
        # quaternion calculation using local fk
        fk_ee_quat = angles_conversion.rotation2quat(sol.end_eff_rot)
        print(f"ee pose => {fk_ee_quat}")
